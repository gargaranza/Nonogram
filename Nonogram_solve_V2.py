...
def solve(Lignes, Colonnes, bis=False):
        ...
        def main_branch(Liste, taille_1, taille_2, Liste_infos, change, grille_coherante):
                ...
                for rangee in Liste_infos:
                        ...
                        if v: print('(G){}*{}(L)={}'.format(len(G_poss), len(L), len(G_poss)*len(L)))
                        for g in G_poss :
                                for l in L :
                                        G_1 = change(g, rangee[1], l)
                                        if grille_coherante(G_1, taille_1, All_listes_possible):
                                                G += [G_1]
                return G
